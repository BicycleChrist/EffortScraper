        ...
        for row_label, row_data in self.table_data.items():
            ...
            for point, lines in over_lines_by_point.items():
                ...
                best_over_by_point[point] = {
                    ...
                    'point': point, 
                    'bookmaker': best_bm,
                    'avg_odds': self.decimal_to_american(avg_decimal_odds),  # Convert average back to American
                    'count': len(lines),
            ...
            if best_over_by_point:
                ...
                over_points.sort(key=lambda x: self.american_to_decimal(x[1]['odds']), reverse=True)
            ...
            if best_under_by_point:
                ...
                # Sort by odds (converting American to decimal first)
                under_points.sort(key=lambda x: self.american_to_decimal(x[1]['odds']), reverse=True)
