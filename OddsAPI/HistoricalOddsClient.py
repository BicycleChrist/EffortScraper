...
class HistoricalOddsWidget(QWidget):
    ...
    async def load_data(self):
        ...
        try:
            ...
            # Create a progress callback to update the UI during the data fetch
            async def progress_callback(current, total):
                if total > 0:
                    progress = int(10 + (current / total) * 40)  # Scale to 10-50% range
                    self.progress_bar.setValue(progress)
                    # Allow UI updates by yielding briefly
            ...
            async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
                # Pass the time range in a consistent format
